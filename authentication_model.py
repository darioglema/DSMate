...
class AuthenticationModel:
    ...
    def login(self, username: str, password: str, max_concurrent_users: Optional[int]=None,
        ...
        if username:
            ...
            # Extraer las credenciales de usuario
            credentials = data.get('credentials', {}).get('usernames', {})
            user_passwords = {username: info['password'] for username, info in credentials.items()}

            normalized_username = username.lower()

            # Verificar si el usuario existe (ignorando mayúsculas/minúsculas)
            matching_username = next((key for key in user_passwords if key.lower() == normalized_username), None)
            ...
            if password != user_passwords[matching_username]:  # Asume que `password` es la entrada del usuario
                st.session_state['authentication_status'] = False
                return False

            # Usuario autenticado correctamente
            st.session_state['email'], st.session_state['name'], st.session_state['roles'] = \
                self._get_user_variables(matching_username)
            st.session_state['authentication_status'] = True
            st.session_state['username'] = matching_username
            self._record_failed_login_attempts(matching_username, reset=True)
            self.credentials['usernames'][matching_username]['logged_in'] = True
            ...
            if self.path:
                Helpers.update_config_file(self.path, 'credentials', self.credentials)
            if callback:
                callback({'widget': 'Login', 'username': matching_username})
            return True
